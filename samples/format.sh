...
for i in "${notes[@]}"
do
	ffmpeg -i ./samples/original/Piano.ff.$i.aiff -af loudnorm,silenceremove=start_periods=1:start_silence=0.01:start_threshold=-20dB,afade=out:st=0.25:d=0.25 -to 0.5 ./samples/formatted/500/$i.mp3
	...
	ffmpeg -i ./samples/original/Piano.ff.$i.aiff -af loudnorm,silenceremove=start_periods=1:start_silence=0.01:start_threshold=-20dB,afade=out:st=1.75:d=0.25 -to 2 ./samples/formatted/2000/$i.mp3
	echo $i.mp3
done